...
- 우리가 일반적으로 이야기하는 학문으로서의 자료구조를 일컫는다
- 자료형이란 컴파일러 또는 인터프리터에게 프로그래머가 데이터를 어떻게 사용하는지를 알려주는 일종의 데이터 속성(Attribute)이다
- 자료형은 자료구조에 비해 훨씬 더 구체적이며, 특정 언어에서 자료형이라 함은 정수(Integer), 실수(Floating-Point Number), 문자열(String) 등 해당 언어에서 지원하는 원시 자료형(Primitive Data Type)까지 포함하는 모든 자료의 유형을 말한다
- 자료구조는 일반적으로 원시 자료형을 기반으로 하는 배열, 연결 리스트, 객체(Object) 등을 말하며, 자료형의 관점에서 보자면 여러 원시 자료형을 조합한 자료구조는 복합 자료형(Composite Data Type)이 된다
- 추상 자료형(ADT)은 자료형에 대한 수학적 모델을 지칭한다
- ADT란 해당 유형의 자료에 대한 연산들을 명기한 것이다
- ADT란 행동만을 정의할 뿐 실제 구현 방법은 명시하지 않는다
- 이런 점에서 자료구조와 다르다
- 객체 지향 프로그래밍(OOP)에서 추상화(Abstraction)를 떠올리면 된다
- 추상화는 필수적인 속성만 보여주고, 불필요한 정보를 감추는 것을 의미하는데, 이 처럼 인터페이스만 보여주고 실제 구현은 보여주지 않는다는 점에서 ADT는 OOP의 추상화와 비슷한 개념이라 할 수 있다
...
- 파이썬 리스트의 가장 좋은 점은 다양한 기능을 제공하는데 있다
- 리스트는 스택과 큐에서 사용 사용한 모든 연산을 함께 제공한다
- 리스트의 O(1) 연산
  - 마지막에 요소 추가 .append()
  - 마지막 요소 제거 .pop()
  - 원하는 인덱스 조회 list(index)
- 리스트의 O(n) dustks
  - 요소 삭제
  - 첫 번째 요소 추출 pop(0)
- 따라서 리스트에 주로 큐의 연산을 사용할 때는 주의가 필요하다
- 이 경우 Deque 같은 자료형으로 성능을 높일 수 있다
- 리스트 주요 연산 시간 복잡도
  - len(a)
    - O(1)
    - 전체 요소의 개수를 리턴
  ...
  - a[i:j]
    - O(k)
    - i부터 j까지 슬라이스의 길이만큼의 k개의 요소를 가져온다. 이 경우 각체 k개에 대한 조회가 필요하므로 O(k)이다
  - elem in a
    - O(n)
    - elem 요소가 존재하는지 확인한다. 처음부터 순차 탐색이므로 n만큼 시간이 소요된다
  - a.count(elem)
    - O(n)
    - elem 요소의 개수를 리턴한다
  - a.index(elem)
  ...
  - a.pop(0)
    - O(n)
    - 리스트 첫번쨰 요소를 추출한다. 큐의 연산이다.
    - 이 경우 전체 복사가 필요하므로 O(n)이다
    - 큐의 연산을 주로 사용한다면 리스트보다는 O(1)에 가능한 데크(Deque)를 권장한다
  - del a[i]
    - O(n)
    - i에 때라 다르다. 최악의 경우 O(n)이다
  - a.sort()
    - O(nlogn)
    - 정렬한다. 팀소트(Timsort)를 사용하며, 최선의 경우 O(n)에도 실행될 수 있다
  - min(a), max(a)
    - O(n)
...
- 특정인덱스에 값을 추가
- 다양한 자료형을 단일 리스트에 관리
  - a.append('Hello') // [1, 2, 3, 5, 4, 'Hello']
  - a.append(True) // [1, 2, 3, 5, 4, 'Hello', True]
  - 다른 언어들에 비해 부가 처리가 필요 없으면 유연하다
- 값 가져오기
  - a[3] // 5
- 슬라이싱
  - 간결함과 강력함을 자랑하는 기능
  ...
  - 문자열에 유용하게 활용되는 기능
  - a[1:3] // [2, 3]
  - 다른 언어들은 반복문을 구성하고 순회하면서 값을 출력해야하는데, 파이썬은 시작 인덱스와, 종료 인덱스를 설정해 간단히 해당하는 값을 추출할 수 있다
  - 시작 인덱스 생략
    - a[:3] // [1,2,3]
  - 종료 인덱스 생략
    - a[4:] // [4, 'Hello', True]
  ...
  - 특정 수만 큼 뛰어서 값 가져오기
    - a[1:4:2] // [2, 5]
    - 세 번째 파라미터를 부여하면 단계(Step)의 의미로 n만큼 건너 뛰며 값을 가져온게 된다
  - 존재하지 않는 인덱스를 조회할 경우 IndexError가 발생한다
    - a[9] // list index out of range
  - 예외 처리
    - try:
      print(a[9])
      except IndexError:
      print('존재하지 않는 인덱스')
  - 리스트에서 요소를 삭제하는 방법은 크게 2가지 이다
    - 인덱스로 삭제하기
      - a = [1, 2, 3, 5, 4, 'Hello', True]
      - del a[1] // [1, 3, 5, 4, 'Hello', True]
    - 값으로 삭제하기
      - a = [1, 2, 3, 5, 4, 'Hello', True]
      - a.remove('Hello') // [1, 2, 3, 5, 4, True]
  - Pop 함수를 사용하면 추출로 처리된다
    - a = [1, 2, 3, 5, 4, 'Hello', True]
    - a.pop(3) // 5
...
- 파이썬의 리스트는 연속된 공간에 요소를 배치하는 배열의 장점과 다양한 타입을 연결해 배치하는 연결 리스트의 장점을 모두 취한 듯한 형태를 띤다
- 리스트는 이처럼 객체로 되어 있는 모든 자료형을 다음과 값은 포인터로 연결한다
- 파이썬의 모든 것이 객체이며, 파이썬 리스트는 이들 객체에 대한 포인터 목록을 관리하는 형태로 구현되어 있다
- 포인터 목록을 관리하는 형태이기 때문에 다양한 자료를 한 리스트에 담을 수 있다
- 결국 각각의 객체에 대한 참조로 구현되어 있어 인덱스를 조회하는 데에도 모든 포인터의 위치를 찾아가서 타입 코드를 확인하고 값을 일일이 살펴봐야 하는 등 추가적인 작업이 필요해 속도 측명에서 불리하다

# 딕셔너리

- 키/값 구조로 이루어진 자료형
- 순서가 유지되며, 내부적으로는 해시 테이블로 구현되어 있다
- 숫자뿐만 아니라 해쉬할 수 있다면 불변 객체를 모두 키로 사용할 수 있다
- 이 과정을 해싱이라고 하며, 해시 테이블을 이용해 자료를 저장한다
- 해시 테이블은 다양한 타입을 키로 지원하면서도 입력과 조회 모두 O(1)에 가능하다
- 최악의 경우 O(n)이 될 수 있으나, 대부분의 경우 훨씬 더 빨리 실행되며, 분할 상환 분석(Amortized Analysis)에 따른 시간 복잡도는 O(1)이다
- 주요 연산 시간 복잡도
  - len(a)
    - O(1)
  - a[key]
  ...
  - key in a
    - O(1)
- 원래 딕셔너리는 순서가 보장되지 않지만 파이썬 3.7 이상에서는 내부적으로 인덱스를 이용해 입력 순서를 유지하도록 개선됐다
- 파이썬 3.6 이하에서는 순서가 보장되는 collections.OrderedDic()을 비룟해, 조회 시 항상 리폴트 값을 생성해 키 오류를 방지하는 collections.defaultdict(), 요소의 값을 키로 하고 개수를 값 형태로 만들어 카운팅하는 collections.Counter() 등이 있다
...
- defaultdict
  - 객체에 존재하지 않는 키를 조회했을 때, 에러 메시지를 출력하는 대신 디폴트 값을 기준으로 해당 키에 대한 딕셔너리 아이템을 생성해준다
  - a = collections.defaultdict(int)
  - a['A'] = 5
  - a['B'] = 4
  - defaultdict(<class 'int'>, {'A': 5, 'B': 4})
  - a['C'] += 1 // 존재 하지 않는 키에 +1 원래 라면 KeyEror 지만 'C': 1 추가해준다 (디폴트 값 0 기준)
- Couter
  - 아이템에 대한 개수를 계산해 딕셔너리로 리턴
  - a = [1,2,3,4,5,5,5,6,6]
  - b = collections.Counter(a)
  - Couter({5:3, 6:2, 1:1, 2: 1, 3: 1, 4: 1})
  - type(b) // <class 'collections.Counter'>
  - 가장 높은 빈도수 추출
    - b.most_common(2) // [(5,3), (6,2)] 가장 빈도가 높은 2개 추출
- OrderedDict 객체
  - 내부적으로 인덱스를 사용하여 순서가 보장되는 dictionary
  - 코딩테스트시 하위 호환 및 인터프린터 버전의 이유로 사용에 주의해야 한다

# 타입 선언
- 파이썬에서 타입을 선언하는 문법은 타입의 이름을 지정하는 방법이 있지만, 기호를 사용해서 편하게 처리할 수 있다
  - a = list()
  - type(a) // <class 'list'>
  - type([]) // <class 'list'>
  - type(()) // <class 'tuple'>
  - type({}) // <class 'dict'>
  - type({1}) // <class 'set'>
  - dictionary와 set은 {} 같은 기호를 사용하지만 키의 유무에 따라 다른 자료형이 된다